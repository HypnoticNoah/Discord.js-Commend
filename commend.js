...
module.exports.run = async (bot, message, args) => {
  if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply("Sorry you don't have permission to commend someone!").then(m => {m.delete(15000)});
  let cUser = message.guild.member(message.mentions.users.first()) || message.guild.members.get(args[0])
  if(!cUser) return message.reply("Can't find the user your trying to commend!").then(m => {m.delete(15000)});
  ...
  let commendchannel = message.guild.channels.find(channel => channel.name === 'bot-logs');
  if(!commendchannel) return message.reply("Couldn't find the channel!");
